...
try:
    ...
    offsets = {
        'client.dll': {
            'dwEntityList': 0x18C7D88,
            'dwLocalPlayerPawn': 0x17371A0,
            'dwViewMatrix': 0x1886D70,
            'dwViewAngles': 0x1AAE880
        }
    }
    client_dll = {
        'client.dll': {
            'classes': {
                ...
                'CSkeletonInstance': {'fields': {'m_modelState': 0x160}},
                'CCSPlayerController': {'fields': {'m_hPlayerPawn': 0x7E4, 'm_sSanitizedPlayerName': 0x720}}
            }
        }
    }
...
def create_window():
    ...
    if not hWnd or not hdc:
        raise RuntimeError("Failed to create window or get DC")
...
dpg.create_context()
viewport = dpg.create_viewport(title='ColoramaHack', width=625, height=600, resizable=False, decorated=False, clear_color=[40, 40, 40, 200])
...
def close_program():
    global hdc, hWnd, buffer_hdc, buffer_bmp, pm, update_thread
    ...
    try:
        ...
        print("Program fully terminated.")
...
def update_color(sender, app_data, user_data):
    if app_data is not None and isinstance(app_data, (list, tuple)) and len(app_data) >= 3:
        ...
        print(f"Updated {user_data}: {config[user_data]}")
...
def load_config(sender, app_data, user_data):
    ...
    if os.path.exists(config_path):
        with open(config_path, 'r') as f:
            ...
            print(f"Loaded config: {user_data}")
            print(f"Loaded team_color: {config['team_color']}")
            print(f"Loaded enemy_color: {config['enemy_color']}")
            print(f"Loaded name_health_color: {config['name_health_color']}")
            print(f"Loaded fov_color: {config['fov_color']}")
            print(f"Loaded bone_color: {config['bone_color']}")
...
with dpg.window(label="ColoramaHack", width=625, height=600, no_title_bar=True, no_move=True, no_resize=True, pos=[0, 0]) as menu_window:
        ...
        with dpg.tab(label="Misc"):
            with dpg.group(horizontal=True):
                ...
                with dpg.group():
                    ...
                    dpg.add_spacer(height=20)
                with dpg.group():
                    dpg.add_text("Made By SA1DEN", pos=[12, 542])
                    dpg.add_text("Version 1.0.0", pos=[12, 565])
        ...
        with dpg.tab(label="Config"):
            with dpg.group(horizontal=True):
                with dpg.group():
                    ...
                    dpg.add_spacer(height=20)
                with dpg.group():
                    with dpg.child_window(height=400, tag="config_list"):
                        update_config_list()
...
def connect_to_game() -> bool:
    ...
    try:
        ...
        print(f"Connected to CS2. Client base: {hex(client)}")
        return True
    except Exception as e:
...
def get_closest_enemy_target(local_pos):
    ...
    try:
        ...
        print(f"Failed to get local position: {e}")
        return None, None
...
def draw_esp_and_aim():
    ...
    if not config['esp_enabled'] and not config['aimbot_enabled'] or pm is None or client is None or hdc is None or hWnd is None or buffer_hdc is None:
        return
        ...
        for i in range(64):
            try:
                ...
                try:
                    ...
                    print(f"Error reading player info for entity {i}: {e}")
                    health = 100
                    player_name = f"Player_{i}"
                ...
                print(f"ESP entity error for index {i}: {e}")
                continue
...
def update():
    global hdc, hWnd, buffer_hdc, update_thread
    try:
        ...
        if hWnd and win32gui.IsWindow(hWnd):
            ...
            print("Window handle invalid, recreating...")
            create_window()
    except Exception as e:
