...
def on_click(x, y, button, pressed):
    if button == mouse.Button.left and state.config['aim_key'] == 'left_mouse':
        state.aim_key_pressed = pressed
    elif button == mouse.Button.right and state.config['aim_key'] == 'right_mouse':
        state.aim_key_pressed = pressed
...
def update_gui_from_config():
    ...
    dpg.set_value("aim_key_selector", "Left" if state.config['aim_key'] == 'left_mouse' else "Right")
...
with dpg.window(label="ColoramaHack", width=900, height=400, no_title_bar=True, no_move=True, no_resize=True, pos=[0, 0]):
    with dpg.theme() as globalTheme:
        with dpg.theme_component(dpg.mvAll):
            ...
            dpg.add_theme_color(dpg.mvThemeCol_SliderGrabActive, (0, 200, 200, 255))
            dpg.add_theme_color(dpg.mvThemeCol_Tab, (50, 50, 60, 255))
            dpg.add_theme_color(dpg.mvThemeCol_TabHovered, (80, 80, 100, 255))
            dpg.add_theme_color(dpg.mvThemeCol_TabActive, (100, 100, 120, 255))
    ...
    dpg.add_text("ColoramaHack", color=(0, 255, 255, 255))
    ...
    with dpg.tab_bar():
            ...
            with dpg.group(horizontal=True):
                with dpg.group():
                    ...
                    dpg.add_radio_button(
                        items=["Left", "Right"],
                        default_value="Left" if state.config['aim_key'] == 'left_mouse' else "Right",
                        callback=lambda s, a: state.config.update({'aim_key': 'left_mouse' if a == "Left" else 'right_mouse'}),
        ...
        with dpg.tab(label="Misc"):
            with dpg.group(horizontal=True):
                with dpg.group():
                    dpg.add_checkbox(label="Show FOV Circle", default_value=state.config['show_fov'], callback=toggle_show_fov, tag="show_fov")
                    ...
                    dpg.add_checkbox(label="Bunny Hop (don't work right now)", default_value=state.config['bhop_enabled'], callback=toggle_bhop, tag="bhop_enabled")
                ...
                with dpg.group():
                    ...
                    dpg.add_spacer(height=235)
            dpg.add_text("Made By SA1DEN | Version 1.0.2", color=(150, 150, 150, 255))
